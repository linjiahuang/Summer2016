import sys
sys.path.insert(0, '/home/jiahuang/Dropbox/Princeton/Sophomore Summer/Summer Project/Code')
...
#Global variables for the metropolis Hasting algorithm
b = 1 
def mcmc(mu_0):
	mu_1 = np.random.normal(mu_0, b**2)
	r = min(normal_null(mu_0, mu_1), 1)
...
def normal_null(mu_0, mu_1):
	exponent = -1 * (mu_0**2 + mu_1**2)/(2 * (Sigma_1**2))
	return math.exp(exponent)

def normal_alt(mu_0, mu_1, x_j):
	exponent = -1 * (mu_0**2 + mu_1**2)/(2 * (Sigma_0**2 + x_j**2 * Sigma_1**2))
	return math.exp(exponent)
